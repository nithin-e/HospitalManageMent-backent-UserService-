import { StatusUpdateResponse } from "../../interfaces/types";
import {
  DoctorFormData,
  DoctorApplicationResponse,
} from "../../interfaces/types";
import { IDoctorRepository } from "../../repositories/interface/IDoctorRepository";
import { IDoctorService } from "../interface/IDoctorServices";
...
export default class ApplyDoctorService implements IDoctorService {
  ...
  applyForDoctor = async (
    ...
    try {
      ...
      const profileImageUrl =
        documentUrls && documentUrls.length > 0 ? documentUrls[0] : null;
      const medicalLicenseUrl =
        documentUrls && documentUrls.length > 1 ? documentUrls[1] : null;
      ...
      const newDoctorData = {
        ...
        medicalLicenseUrl,
        agreeTerms:
          typeof agreeTerms === "string" ? agreeTerms === "true" : !!agreeTerms,
      };

      const response = await this._applyDoctorRepo.applyForDoctor(
        newDoctorData
      );
  ...
  updateDoctorStatusAfterAdminApproval = async (
    ...
    try {
      const response =
        await this._applyDoctorRepo.updateDoctorStatusAfterAdminApproval(email);
