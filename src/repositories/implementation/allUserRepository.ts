...
import {
  IUserRepository,
  SearchUserResponse,
} from "../interface/IAllUsersRepository";
import type { User as UserType } from "../../entities/user_schema";
import { DoctorDb } from "../../entities/doctor_schema";
import { BaseRepository } from "./baseRepository";
import { SearchDoctorResponse, SearchParamss } from "src/interfaces/types";
import { SearchParams } from "src/entities/user_interface";
...
export default class FetchAllDataRepository
  ...
  implements IUserRepository
{
  ...
  async searchDoctors(params: SearchParamss): Promise<SearchDoctorResponse> {
    try {
      ...
      const [doctors, totalCount, approvedCount, pendingCount, declinedCount] =
        await Promise.all([
          DoctorDb.find(query)
            .sort(sortObj)
            .skip(skip)
