...
import { IRegisterService } from "../../services/interface/IRegistretionService";
...
export default class RegistrationController {
  ...
  signup = async (
    ...
    try {
      const response = await this._registrationService.userRegistration(
        userData
      );
      ...
      const userMessage = {
        ...
        phoneNumber:
          response.user.phoneNumber || response.user._doc?.phoneNumber,
        id: (response.user._id || response.user._doc?._id)?.toString(),
        createdAt: (
          response.user.createdAt || response.user._doc?.createdAt
        )?.toISOString(),
  ...
  checkUser = async (
    ...
    try {
      ...
      const response = await this._registrationService.checkUser(
        email,
        phoneNumber
      );
