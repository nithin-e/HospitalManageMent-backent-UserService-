...
import { IDoctorService } from "../../services/interface/IDoctorServices";

type ApplyDoctorCall = grpc.ServerUnaryCall<
  ApplyDoctorRequest,
  ApplyDoctorResponse
>;
type UpdateStatusCall = grpc.ServerUnaryCall<
  UpdateDoctorStatusAfterAdminApproveRequest,
  UpdateDoctorStatusAfterAdminApproveResponse
>;
...
export default class ApplyDoctorController {
  ...
  applyForDoctor = async (
    ...
    try {
      const response = await this._applyDoctorService.applyForDoctor(
        doctorData
      );
      const grpcResponse: ApplyDoctorResponse = {
        ...
        message:
          response.message || "Doctor application submitted successfully",
      };
  ...
  UpdateDoctorStatusAfterAdminApprove = async (
    ...
    try {
      const response =
        await this._applyDoctorService.updateDoctorStatusAfterAdminApproval(
          email
        );
