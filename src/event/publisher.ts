...
class RabbitMQPublisher {
    ...
    static async initialize(): Promise<void> {
        ...
        if (this.ch && this.conn) {
            try {
                ...
                console.log(
                    '🔄 Existing connection is dead, reinitializing...'
                );
            }
        }
    ...
    private static async _doInitialize(): Promise<void> {
        try {
            ...
            await this.ch.assertExchange('healNova', 'topic', {
                durable: true,
            });
    ...
    static async publish(routingKey: string, data: any): Promise<void> {
        ...
        while (retryCount < maxRetries) {
            try {
                ...
                if (!this.ch) {
                    throw new Error(
                        'RabbitMQ channel not available after initialization'
                    );
                }
                ...
                retryCount++;
                console.error(
                    `❌ Failed to publish to ${routingKey} (attempt ${retryCount}/${maxRetries}):`,
                    error
                );
